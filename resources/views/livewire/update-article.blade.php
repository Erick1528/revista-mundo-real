    ...
    <form action="" class="space-y-4">
        ...
        <div class="border border-gray-lighter">
            ...
            <div id="section-image"
                ...
                <div class="space-y-2 grid grid-cols-1 gap-3">
                    ...
                    @if (!$image || (is_string($image) && $image))
                        <!-- Loading spinner -->
                    ...
                    @if ($image && (is_object($image) || is_string($image)))
            ...
            <div id="section-classification"
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <!-- Sugerencias -->
                        @if (isset($this->searchSuggestions) && count($this->searchSuggestions) > 0)
                            <div
                            ...
                            @foreach ($related_articles as $index => $article)
                                <div class="flex items-start justify-between p-4 bg-gray-50 border border-gray-200">
                                    ...
                                    <button type="button" wire:click="removeRelatedArticle({{ $index }})"
                                        class="ml-3 text-gray-400 hover:text-red-500 transition-colors shrink-0">
        ...
        <div class="flex flex-col sm:flex-row gap-4 pt-4">
            <button wire:click="update" type="button"
                ...
                <!-- Texto del botón -->
                <span wire:loading.remove wire:target="update">Actualizar Artículo</span>
                <span wire:loading wire:target="update">Actualizando...</span>
            </button>

            <button wire:click.prevent="saveDraft" type="button"
        ...
        <div class="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50 p-4"
            x-data="{
                init() {
                    ...
                    closeModal() {
                        $wire.closeCancelModal();
                        this.destroy();
                    }
            }" x-init="init()" x-on:click.self="closeModal()">
            <div class="bg-white max-w-md w-full mx-4 border border-gray-lighter shadow-xl" x-on:click.stop>
                <div class="px-6 py-6">
                    ...
                    <p class="text-sm font-opensans text-gray-light text-center mb-6">
                        Si cancelas ahora, <strong class="text-primary">se perderán todos los cambios</strong> que has
