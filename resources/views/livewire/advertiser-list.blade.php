<div class="px-4 sm:px-10 lg:px-[120px] py-12 max-w-[1200px] mx-auto w-full">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
        <h2 class="font-serif text-2xl sm:text-3xl text-primary">Gestionar Anunciantes</h2>
    ...
    <div class="mb-8 space-y-4">
        <div class="flex items-center justify-between">
            <span class="text-sm text-gray-light font-opensans">Filtrar anunciantes</span>
            <div class="flex items-center gap-2 sm:gap-3">
                ...
                <a href="{{ route('advertisers.trash') }}"
    ...
    <div class="space-y-3 sm:space-y-4">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                    <div class="min-w-0 flex-1 flex items-center gap-3">
                        @if ($advertiser->logo_path)
                            <img src="{{ $advertiser->logo_url }}" alt="{{ $advertiser->name }}" class="h-10 w-auto object-contain border border-gray-200 shrink-0">
                        @endif
                        <div>
                            <h3 class="font-serif text-lg sm:text-xl text-primary break-words">{{ $advertiser->name }}</h3>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-2 items-center shrink-0">
                        ...
                        <button type="button" wire:click="openDeleteModal({{ $advertiser->id }})"
        ...
        @empty
            <div class="border border-gray-lighter p-6 sm:p-8 text-center">
                @if($hasActiveFilters)
                    <p class="font-opensans text-sm sm:text-base text-gray-light mb-4">No se encontraron anunciantes con los filtros aplicados.</p>
                    <button type="button" wire:click="clearFilters"
                        class="text-dark-sage hover:text-primary transition-colors font-opensans text-sm">
                        Limpiar filtros
                    </button>
                @else
                    <p class="font-opensans text-sm sm:text-base text-gray-light mb-4">No hay anunciantes. Crea el primero para usarlo en artículos y anuncios.</p>
                    <a href="{{ route('advertisers.create') }}"
        ...
        <div class="bg-white shadow-xl max-w-md w-full p-4 sm:p-8 mx-4 border border-gray-lighter" @click.stop>
            @if($deleteModalBlockedByUse)
                <div class="text-center mb-4 sm:mb-6">
                    ...
                    <p class="text-xs sm:text-sm font-opensans text-gray-light leading-relaxed mb-3">
                        El anunciante <strong class="font-semibold text-primary">{{ $selectedAdvertiserName }}</strong> está en uso en anuncios o en artículos patrocinados. Deja de usarlo en esos anuncios o artículos si quieres moverlo a la papelera.
                    </p>
                </div>
            ...
            @else
                <div class="text-center mb-4 sm:mb-6">
                    ...
                    <h2 class="text-xl sm:text-2xl font-serif text-primary mb-3 sm:mb-4">¿Mover este anunciante a la papelera?</h2>
                    <p class="text-xs sm:text-sm font-opensans text-gray-light leading-relaxed mb-3">
                        El anunciante <strong class="font-semibold text-primary">{{ $selectedAdvertiserName }}</strong> se moverá a la papelera.
                    </p>
