<div class="px-4 sm:px-10 lg:px-[120px] py-12 max-w-[1200px] mx-auto w-full">
    ...
    <p class="text-sm text-gray-light font-opensans mb-8">
        Usuarios dados de baja. Puedes restaurarlos para que vuelvan a aparecer en el listado y puedan iniciar sesión de nuevo.
    </p>
    ...
    <div class="space-y-6">
            ...
            <div class="border border-gray-lighter p-4 sm:p-6 hover:bg-sage/30 transition-all duration-200">
                <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
                    <div class="min-w-0 flex-1">
                        <h3 class="text-lg sm:text-xl font-serif text-primary mb-1">{{ $user->name }}</h3>
                        <p class="text-gray-light font-opensans text-sm mb-1">{{ $user->email }}</p>
                        <p class="text-xs text-gray-light font-opensans">
                            {{ $roles[$user->rol] ?? $user->rol }}
                            · Eliminado {{ $user->deleted_at->locale('es')->diffForHumans() }}
                        </p>
                    </div>
                    <div class="flex flex-wrap gap-2 items-center shrink-0">
                        ...
                        <button type="button" wire:click="openForceDeleteModal({{ $user->id }})"
        ...
        @empty
            <div class="border border-gray-lighter p-6 sm:p-8 text-center">
                ...
                <p class="text-gray-light font-opensans">No hay usuarios eliminados</p>
            </div>
        @endforelse
    </div>
        ...
        <div class="bg-white shadow-xl max-w-md w-full p-4 sm:p-8 mx-4" @click.stop>
            <div class="text-center mb-4 sm:mb-6">
                ...
                <p class="text-xs sm:text-sm font-opensans text-gray-light leading-relaxed mb-3">
                    El usuario <strong class="font-semibold text-primary">{{ $selectedUserName }}</strong> se borrará para siempre, junto con su foto de perfil. Esta acción no se puede deshacer.
                </p>
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <button type="button" wire:click="confirmForceDeleteUser"
