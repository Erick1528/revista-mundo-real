<div class="px-4 sm:px-10 lg:px-[120px] py-12 max-w-[1200px] mx-auto w-full">
    ...
    <p class="text-sm text-gray-light font-opensans mb-8">
        Portadas eliminadas. Puedes restaurarlas para que vuelvan al listado o eliminarlas permanentemente.
    </p>
    ...
    <div class="space-y-3 sm:space-y-4">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="font-opensans text-xs sm:text-sm text-gray-light mt-1">
                            Eliminado {{ $cover->deleted_at->locale('es')->diffForHumans() }}
                            @if ($cover->creator)
                                · Creado por {{ $cover->creator->name }}
                            @endif
                        </p>
                    </div>
                    <div class="flex flex-wrap gap-2 items-center shrink-0">
                        <button type="button" wire:click="restoreCover({{ $cover->id }})"
                        ...
                        <button type="button" wire:click="openForceDeleteModal({{ $cover->id }})"
        ...
        @empty
            <div class="border border-gray-lighter p-6 sm:p-8 text-center">
                ...
                <p class="text-gray-light font-opensans">No hay portadas eliminadas</p>
            </div>
        @endforelse
    </div>
        ...
        <div class="bg-white shadow-xl max-w-md w-full p-4 sm:p-8 mx-4 border border-gray-lighter" @click.stop>
            <div class="text-center mb-4 sm:mb-6">
                ...
                <p class="text-xs sm:text-sm font-opensans text-gray-light leading-relaxed mb-3">
                    La portada <strong class="font-semibold text-primary">{{ $selectedCoverName }}</strong> se borrará para siempre. Esta acción <strong class="font-semibold text-red-500">no se puede deshacer</strong> y es irreversible.
                </p>
                <p class="text-xs sm:text-sm font-opensans text-gray-light leading-relaxed">
                    ¿Estás seguro de que deseas eliminarla definitivamente?
                </p>
            </div>
