            ...
            <div id="section-data"
                ...
                <div class="space-y-2">
                    ...
                    @if ($logo && is_object($logo) && method_exists($logo, 'temporaryUrl') && $this->logoPreviewUrl)
                        ...
                        <p class="text-xs text-gray-500 font-opensans mt-1">Se reemplazará al guardar. Quita la vista previa para mantener el logo actual.</p>
                    @else
                        @if ($advertiser->logo_path)
                            <div class="mb-3 flex items-center gap-3">
                                <div class="flex items-center justify-center">
                                    <img src="{{ $advertiser->logo_url }}" alt="{{ $advertiser->name }}" class="max-h-16 w-auto object-contain">
                                </div>
                                <span class="text-xs text-gray-500 font-opensans">Logo actual. Sube otro para reemplazarlo.</span>
                            </div>
                        @endif
